...
function oh_msg_triage(?string &$err = null): array {
    ...
    foreach ($emails as $m) {
        ...
        $cands[] = ['kanal' => 'email', 'key' => $key, 'from' => $m['from'] ?? '', 'text' => $m['subject'] ?? '', 'ts' => $m['ts'] ?? time()];
    }
    foreach ($wa as $w) {
    ...
    $system = "Du bist Kaan, der Kommunikations-Manager von OH Haustechnik (Elektriker Nürnberg). Ziel der Firma: in 5 Monaten Richtung 1.000.000 € Umsatz. "
        ...
        . "UNWICHTIG = Standard, kann automatisch beantwortet werden (Eingangsbestätigung, einfache Rückfrage, Terminbestätigung, Werbung/Newsletter, Spam). "
        . "Gib für JEDE Nachricht: kategorie (genau eines: 'Antwort notwendig','Freigabe erforderlich','Umsatzrelevant','Risiko','Info'), "
    ...
    foreach ($rows as $r) {
        ...
        $kat = $wichtig ? ($r['kategorie'] ?? 'Antwort notwendig') : 'Auto-Antwort vorbereitet';
        $id = oh_freigabe_add([
            'dedup'     => $c['key'],
            'agent'     => 'kaan',
            'kanal'     => $c['kanal'],
...
function oh_agenten_runde(?string &$err = null): ?array {
    ...
    // Nachricht landet im Gedächtnis des Empfängers (Fundament für echte Postfächer/Kommunikation)
    foreach (($data['nachrichten'] ?? []) as $msg) {
        $an  = $msg['an'] ?? '';
        $von = $msg['von'] ?? '';
        $txt = $msg['text'] ?? '';
        if ($an !== '' && $txt !== '') oh_agent_mem_add($an, "Nachricht von $von: $txt", 'nachricht');
    }
...
function oh_mission(): string {
    return "MISSION (gilt für ALLE Mitarbeiter): OH Haustechnik in 5 Monaten Richtung 1.000.000 € Umsatz bringen. "
         . "Hochwertige Aufträge (Komplett-/Altbausanierung, Zähleranlagen, Smart-Home) bringen am meisten. "
         . "Triff jede Einschätzung und jeden Vorschlag mit genau diesem Ziel im Kopf.\n";
}
